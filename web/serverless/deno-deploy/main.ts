...
router.options(routes.ROOT, cors()).get(routes.ROOT, (ctx) => {
  ctx.response.body = routes;
});

router.options(routes.GET_ROOM, cors()).post(routes.GET_ROOM, async (ctx) => {
  try {
...
router
  .options(routes.ADD_ENCRYPTED_SHARE, cors())
  .post(routes.ADD_ENCRYPTED_SHARE, async (ctx) => {
    try {
      const body = (await ctx.request.body({ type: "json" }).value) as
...
router
  .options(routes.SET_MIN_SHARE_COUNT, cors())
  .post(routes.SET_MIN_SHARE_COUNT, async (ctx) => {
    try {
      const body = (await ctx.request.body({ type: "json" }).value) as
